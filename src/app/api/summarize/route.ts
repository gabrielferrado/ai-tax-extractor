...
async function startResumableUpload(file: File): Promise<string | null> {
  console.log('Starting resumable upload for file:', { name: file.name, type: file.type, size: file.size });
  ...
  if (response.ok) {
    ...
    console.error("Gemini: Failed to start resumable upload", errorText);
    return null;
  }
}

async function uploadFile(uploadUrl: string, file: File, buffer: Buffer): Promise<string | null> {
  console.log('Uploading file to Gemini:', { uploadUrl, fileType: file.type, bufferSize: buffer.length });
  ...
  if (response.ok) {
    ...
    console.log('File uploaded successfully, got file URI:', data?.file?.uri);
    return data?.file?.uri;
  } else {
    const errorText = await response.text();
    console.error("Gemini: Failed to upload file", errorText);
    return null;
  }
}

async function callGenerateContent(fileUri: string, fileType: string): Promise<string | null> {
  console.log('Calling Gemini to generate content:', { fileUri, fileType });
  ...
  if (response.ok) {
    ...
    const errorText = await response.text();
    console.error("Gemini: Failed to generate content", errorText);
    return null;
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log('Processing PDF file:', { name: file.name, type: file.type, size: file.size });
    ...
    if (!summary) {
      console.error('Failed to get summary from Gemini');
    ...
    console.log('Successfully processed request, returning summary of length:', cleanedSummary.length);
    return NextResponse.json({ summary: cleanedSummary });
  } catch (error) {
    console.error('Error in /api/summarize (Gemini):', error);
